...
A mesclagem de interrupções é um conjunto de parametros do \textit{driver} de redes que podem ser configurados.

Ele serve para reduzir a quantidade de interrupções na transmissão/recepção de pacotes dentro de um intervalo de tempo ou número de pacotes.
Normalmente ele é usado quando a transmissão/recepção de pacotes é muito rápida no meio físico \cite{dong2011optimizing}. Quando isso acontece, a quantidade de interrupções devido a chegada de pacotes é muito grande e acaba sobrecarregando o processador e a memória. 

No contexto da virtualização de rede, ele 
