...
O \textit{dom0} ou domínio zero é a primeira máquina virtual iniciada pelo XEN. Ela tem certos privilégios que as outras máquinas virtuais não têm como iniciar novas máquinas e acessar o hardware diretamente. O \texttt{domU} ou domínio do usuário são máquinas virtuais que, por padrão, não tem alguns privilégios que o \texttt{dom0} tem como o acesso direto ao \textit{hardware}. Assim, é necessário um mecanismo para conseguir acessar o dispositivo de red \cite{xenguide}.
...
Na configuração de roteador o \texttt{dom0} cria uma ligação entre ele e cada \texttt{domU}. Rotas de cada domU são adicionados na tabela de roteamento do \texttt{dom0}, nesse caso o \textit{IP} de cada \textit{domU} tem que ser estático.\\


\textit{james2004performance} fez um experimento comparando a ponte virtual e o roteador virtual. Os resultados foram semelhantes tanto na largura de banda como na latência e no uso do processador. 
...
Esse canal evita que cada pacote tenha que ser copiado de um domínio a outro. Para tal, o textit{domU} compartilha algumas páginas de sua memória e informa a referência delas por esse canal para o outro domínio mapea-las em seu espaço de endereço. Quando algum domínio coloca algum pacote nessas páginas uma notificação é enviada para o outro domínio.

Esse mecanismo consiste de notificações de evento e um \textit{buffer} de descrição em anel. 
...
A mesclagem de interrupções é uma das propostas da literatura para resolver esse problema \cite{salah2007coalesce}.
...
O NAPI(New API) \cite{Corbet:2005:LDD:1209083} é uma interface para usar técnicas de mesclagem de interrupções para dispositivos de rede no núcleo do Linux. O objetivo dele é reduzir a carga extra do processamento na recepção de pacotes de vários dispositivos.
Para isso, no momento em que há uma grande quantidade de tráfego em vários dispositivos de rede, ao invés do \textit{drivers} gerar uma interrupção para cada pacote que recebe, o núcleo desabilita as interrupções e passa a checar continuamente a chegada de pacotes em cada dispositivo e caso o sistema não dê conta de manipular os pacotes, ele passa a jogá-los fora antes de levá-los ao núcleo.
...
O controle de quando ele deve entra ou sair no modo de \textit{polling} e quantos pacotes ele deve pegar por interrupção em cada dispositivo de rede são definidos por um parâmetro chamado ``peso''.
 Com pesos altos, a quantidade de pacotes esperada para gerar uma interrupção ou para entrar em \textit{polling} no dispositivo é maior, enquanto que com pesos baixos, a quantidade de pacotes esperada é menor. \textit{NAPI}.
...
\subsection{Mesclagem de Interrupções na Transmissão}
Tanto a transmissão e a recepção de pacotes podem gerar interrupções com uma frequência grande. 
A transmissão gera uma interrupção quando um pacote é transmitido com sucesso e a recepção gera uma interrupção quando um pacote é recebido.
A diferença entre eles é que enquanto a transmissão pode ser controlar os pacotes que são enviados pelo sistema, a recepção não consegue controlar os pacotes que chegam.
...
Fingindo ter uma \textit{MTU} alta o pacote é segmentado em pedaços grandes e em menor quantidade quando o sistema mandar transmití-lo. Com menos pacotes a quantidade de interrupções por pacote é reduzida. 
Na recepção, o \textit{LRO}(large receive offload) e \textit{GRO}(generic receive offload) \cite{gro} são soluções baseado do \textit{GSO} onde os pacotes são mesclados quando recebidos. O \textit{LRO} agrega todos os pacotes TCP que chegam, mas com possíveis perdas na transformação se por exemplo existe uma diferença nos cabeçalhos no pacote. Já o \textit{GRO} restringe a mesclagem dos pacotes pelos cabeçalhos oque não traz perdas e além disso o \textit{GRO} não é limitado ao TCP. Apesar de conseguir uma mesclagem dos pacotes, como já foi dito, a recepção não pode controlar a chegada dos pacotes oque força a ter que usar uma técnica de mesclagem de interrupção como o \textit{NAPI} para conseguir resegmentar os pacotes.

\subsection{virtualização de rede}
No contexto da virtualização de rede, como foi possível observar na arquitetura da virtualização da rede no \textit{XEN}, muitos passos extras são feitos durante recepção e transmissão de pacotes, incluindo o aumento no número de interrupções. 
...
A mesclagem no \textit{driver} do dispositivo físico é complexa já que afeta o tráfego de pacotes em todas máquinas virtuais e, consequentemente, em todas as aplicações que usam a rede.
Também necessita que a placa de rede tenha suporte a mesclagem. 
...
\subsection{netback}
A mesclagem no \textit{netback}, diferente do \textit{driver} do dispositivo físico afeta apenas as aplicações de uma determinada máquina virtual. 
Pelo \textit{netback} estar no \textit{driver domain} consumindo processamento junto com vários outros \textit{netbacks}, reduzir as interrupções dele aliviaria o processamento da rede por máquina virtual do \textit{driver domain} e poderia permitir que mais máquinas virtuais usem a rede.
...
Pelo \textit{netback} e \textit{netfront} serem virtuais e desacoplados da lógica do \textit{driver} de rede físico, eles podem usar técnicas de mesclagem se tiverem suporte independente do \textit{driver} de rede físico.
...
Os autores perceberam que o pacote passa por duas camadas de \textit{drivers} virtuais de rede antes de chegar no destino. O primeiro é o \textit{driver} de \textit{backend} que fica na ponte e o outro é o driver de \textit{frontend} que está dentro da máquina virtual. 
Considerando estas duas camadas, a combinação de mesclagem de interrupções nas duas causaria um atraso adicional no envio. Nessa pesquisa eles focaram em otimizar os \textit{drivers} virtuais, deixando de fora o \textit{driver} físico.
