...
Essa pesquisa tem como tema técnicas de otimização na virtualização de rede dentro de infraestruturas em nuvem.
...
A computação em nuvem é importante para os negócios e vem crescendo como citam:
Muitas organizações tem investido em computação em nuvem, mais de 150 empresas tem entrado na indústria como fornecedoras de nuvem. No lado dos consumidores de nuvem, um recente pesquisa com mais de 600 companhias pelo InformationWeek revelou que o número de companhias usando computação em nuvem aumentou de 18\% em fevereiro de 2009 para 30\% em outubro de 2010 \cite{goncalvesresource}.
 
Uma infraestrutura com um desempenho baixo do que ela pode obter reduz a quantidade de recursos que ela pode forner. Com menos recursos o ganho monetário poderá ser menor seguindo o modelo ``pague somente quando usa''\cite{armbrust2009above}.
...
A proposta será uma solução se o desempenho em largura de banda e latência de aplicações que usam intensamente a rede tiverem próximas do desempenho de infraestruturas sem virtualização.

Tomando como base a arquitetura da virtualização de rede do XEN\ref{arq}, vemos que existe três \textit{drivers} de rede antes de chegar ao destino: o \textit{driver} do dispositivo físico, o \textit{netback} e o \textit{netfront}.
...
Quando modificamos o driver físico, os requisitos da aplicação pode tanto ser satisfeito como deixar de ser.

Como exemplo, podemos ter duas aplicação as quais uma requer uma baixa latência e baixa largura de banda enquanto que outra requer muito processamento e alta largura de banda. 
...
Quando mesclar, a primeira funcionará mal pois a mesclagem irá provocar um atraso e a segunda funcionará bem porque a mesclagem reduziu o processamento da rede. 

Uma possível solução seria forçar todas as aplicações a terem o mesmos requisitos realocando as máquinas com requisitos de aplicação diferentes para outras infraestruturas. Porém, se existir máquinas com várias aplicações isso não seria possível.
