...
\cite{liu2010evaluating} fez diversos experimentos com virtualizações de E/S baseados em \textit{software} (\textit{virtio}) e em hardware (\textit{SR-IOV}) usando o \textit{hypervisor} KVM. Foram analisadas diversas métricas: a largura de banda, a latência, chamadas para memória, comunicação entre máquinas, tamanho da MTU, IRQ afinitivo e distribuído e \textit{IOMMU}.
 ...
 Na latência, o \textit{virtio} (driver para virtualizar a E/S nas MVs) teve um desempenho muito baixo. A explicação, provada desabilitando a função de mitigação de transmissão, é que o hospedeiro atrasa o pacote para ser mandado em lotes, mas mesmo assim, seu desempenho sem espera ainda perdeu próximo de 20 microssegundo em relação a máquina pura.
 Por outro lado, desabilitar essa função provocaria uma perca de desempenho com quantidade grande de carga de trabalho. Já o \textit{SR-IOV} (single root I/O virtualization) teve um desempenho próximo da máquina pura perdendo apenas alguns microssegundos devido a virtualização da interrupção.
...
Pelos experimentos e propostas lidos, os artigos parecem se dividir em duas categorias: aqueles que propõem modificar o dispositivo de E/S físico para que este esteja ciente da virtualização e aqueles que propõem otimizar a arquitetura de virtualização de E/S atual. Ambos parecem gerar bons resultados em relação ao desempenho de infraestruturas com máquinas puras. 
Apesar disso, as propostas de modificar o dispositivo físico dificultam a migração e a portabilidade da infraestrutura \cite{Santos:2008:BGS:1404014.1404017}. 
Em infraestruturas que usam \textit{OpenVZ} parecem não ter o problema com a virtualização de E/S. Uma das principais causas seria o uso de apenas um núcleo e o compartilhamento da memória entre os domínios.
