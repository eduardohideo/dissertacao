...
\cite{xenbestpractices} sugere algumas práticas para tentar evitar um baixo desempenho em aplicações intensivas de rede/disco no XEN como dedicar exclusivamente um núcleo de processador ou uma quantidade de memória para o \texttt{dom0} e dar mais tempo de processamento ao \texttt{dom0}.
...
Eles também, no final, discutiram a possibilidade de usar a função de agrupamento existentes nos \textit{drivers} das placas de rede modernas. Essa função permite reduzir a quantidade de interrupções causadas pelas placas de rede. Para isso, o \textit{driver} passa a agrupar os pacotes que chegam antes de gerar uma interrupção ao invés de gerá-la no momento em que chega qualquer pacote.


\cite{dong2011optimizing} propuseram otimizações para reduzir a carga extra na virtualização da rede. Uma das otimizações foi agrupar eficientemente as interrupções virtuais e a outra escalar o lado da recepção. O agrupamento de interrupção normalmente é usado quando a transferência de pacotes do meio físico é muito alta. Com uma transferência grande a placa de rede passa a trabalhar intensamente sobrecarregando o processador com interrupções.
...
O agrupamento de interrupções virtuais pode ser feito no \textit{driver} virtual de \textit{backend}(na porte dentro do \texttt{dom0}) ou no \textit{driver} virtual de \textit{frontend}(dentro de um \texttt{domU}).
...
O resultado no experimento de agrupamento de interrupções foi um ganho de até 76\% na largura de banda em relação a configuração padrão e na de escalar o lado da recepção foi até 2,2 vezes mais também na largura de banda.
...
A revisão ajudou a entender melhor a área de virtualização de rede. Diversas formas de melhorar o desempenho foram encontradas, podendo separa-las em 3 categorias: aqueles que propõem modificar o dispositivo de E/S físico para que este esteja ciente da virtualização, os que propõem otimizar a arquitetura de virtualização de E/S atual e por fim os que sugerem modificar os parâmetros que são possíveis de serem configurados e usar ferramentas. Essa separação fez dificultar um pouco a correlacionar os trabalhos propostos.
Numa próxima revisão, seria interessante focar em alguma categoria para facitar a comparação entre pesquisas.
